...
#from http://pypi.python.org/pypi/lovely.jsonrpc
...
def lovely_proxy(url, timeout):
    regex = "http://(?P<username>.*):(?P<password>.*)@.*"
    r = match(regex, url)

    session = proxy.Session(username=r.group("username"), password=r.group("password"))

    return proxy.ServerProxy(url, session=session, timeout=timeout)
...
class Net_manager(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            # failure per the above factor of 5.
            for p in penalties:
                if not p.empty():
                    p.get()
                    p.task_done()
...
class Submitter(Thread):
    def __init__(self, block, nonce, urlindex):
        ...
        self.proxy = lovely_proxy(options.url[self.urlindex], rpc_timeout)
